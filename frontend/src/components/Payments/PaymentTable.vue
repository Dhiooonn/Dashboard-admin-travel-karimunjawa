...
const summary = computed(() => {
  const totalAmount = filteredTransactions.value.reduce(
  ...
  <div class="bg-white rounded-2xl p-6 space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-center gap-4">
      ...
      <div class="flex items-center gap-3 w-full sm:w-auto">
        <DropdownMenu>
          ...
          <DropdownMenuContent
            align="end"
            class="w-36 rounded-xl border-gray-100"
          >
            <DropdownMenuItem
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent
            align="end"
            class="w-40 rounded-xl border-gray-100"
          >
            <DropdownMenuItem
              @click="methodFilter = 'All Methods'"
              class="rounded-lg cursor-pointer"
              >All Methods</DropdownMenuItem
            >
            <DropdownMenuItem
              @click="methodFilter = 'Credit Card'"
              class="rounded-lg cursor-pointer"
              >Credit Card</DropdownMenuItem
            >
            <DropdownMenuItem
              @click="methodFilter = 'Bank Transfer'"
              class="rounded-lg cursor-pointer"
              >Bank Transfer</DropdownMenuItem
            >
            <DropdownMenuItem
              @click="methodFilter = 'PayPal'"
              class="rounded-lg cursor-pointer"
              >PayPal</DropdownMenuItem
            >
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
    ...
    <div class="overflow-x-auto w-full">
      <Table>
        <TableHeader>
          <TableRow
            class="hover:bg-transparent border-t border-b border-gray-100 bg-gray-50/50"
          >
            <TableHead
              class="text-xs font-bold text-gray-500 uppercase tracking-widest h-12 pl-4"
              >TRANSACTION ID</TableHead
            >
            <TableHead
              class="text-xs font-bold text-gray-500 uppercase tracking-widest h-12"
              >CUSTOMER</TableHead
            >
            <TableHead
              class="text-xs font-bold text-gray-500 uppercase tracking-widest h-12"
            ...
            >
            <TableHead
              class="text-xs font-bold text-gray-500 uppercase tracking-widest h-12 pr-4"
              >STATUS</TableHead
            >
          </TableRow>
        </TableHeader>
        ...
        <TableBody>
          ...
          >
            <TableCell class="pl-4 py-4">
              <span class="font-bold text-[#5C5CFF] text-sm">{{
                item.id
              }}</span>
            </TableCell>
            ...
            <TableCell class="py-4">
              <div
                class="flex items-center gap-2 text-gray-600 text-sm flex-nowrap whitespace-nowrap"
              >
                <component
                  :is="getMethodIcon(item.method)"
    ...
    <div
      class="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6 border-t border-gray-100 text-center md:text-left layout-footer"
    >
      ...
      <div class="flex flex-col items-center md:items-start space-y-1">
        ...
        <span class="text-3xl font-black text-[#5C5CFF]">{{
      ...
      <div class="flex flex-col items-center md:items-start space-y-1">
        ...
        >
        <span class="text-3xl font-black text-teal-500">{{
